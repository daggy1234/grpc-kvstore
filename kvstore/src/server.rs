...
use kvstore::{AllItems, AllItemsParams, GetParams, Item,  Success};
...
async fn put(&self, request: Request<Item>) -> Result<Response<Item>, Status> {
    ...
    hm.insert(key, val);
    Ok(Response::new(Item { key: "".to_string(), value: "".to_string() }))

}


async fn delete(&self, _request: Request<GetParams>) -> Result<Response<Success>, Status> {
    unimplemented!()
}

type GetAllItemsStream = ReceiverStream<Result<Item,Status>>;

async fn get_all_items(&self, _request: Request<AllItemsParams>) -> Result<Response<Self::GetAllItemsStream>, Status> {
